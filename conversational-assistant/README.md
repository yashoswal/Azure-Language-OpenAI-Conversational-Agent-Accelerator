...
### Conversational-Assistant: Responsible AI FAQ
- **What is `Conversational-Assistant`?**
    - The `Conversational-Assistant` project provides users with a code-first example on how to augment an existing RAG solution with Azure AI Language functionality. The system takes as input user chat messages (grounded within a specific context). The system orchestrates user chats to Azure AI Langauge CLU or CQA projects, or to a fallback RAG agent. Output is a system chat message, which may be the answer to a question in a CQA project, the result of a linked intent in a CLU project, or a general grounded response from RAG.
...
- **What can `Conversational-Assistant` do?**
    - Overall, the demo provided with this proejct showcases the following chat experience:
        ...
        - Assistant summaries response (what business action was performed, provide answer to question, provide grounded response).

- **What is `Conversational-Assistant`'s intended uses?**
    - The `Conversational-Assistant` project is intended to display the "better together" story when using Azure AI Language and Azure OpenAI. This chat experience includes single-turn chatting, QA, and groundedness. When combined with an existing RAG solution, adding a `UnifiedConversationOrchestrator` can help in the following ways:
        - manual overrides of DSAT examples using CQA.
        - extended chat functionality based on recognized intents/entities using CLU.
        - consistent fallback to original chat functionality with RAG.

- **How was `Conversational-Assistant` evaluated? What metrics are used to measure performance?**
    - `Conversational-Assistant` underwent red teaming RAI procedures to ensure metrics of harmful content and groundedness were met. 

- **What are the limitations of `Conversational-Assistant`? How can users minimize the impact of `Conversational-Assistant`'s limitations when using the system?**
...
- **What operational factors and settings allow for effective and responsible use of `Conversational-Assistant`?**
    - Users provide their own AOAI resource. Depending on the AOAI model provided, accuracy of `Conversational-Assistant` may vary. If you choose to alter the provided example data, ensure that it meets guidelines of responsible AI practices.

- **How do I provide feedback on `Conversational-Assistant`?**
